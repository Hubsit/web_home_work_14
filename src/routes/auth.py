...
async def signup(body: UserModel, background_tasks: BackgroundTasks, request: Request, db: Session = Depends(get_db)):
    ...
    :return: A new user, so we need to add the user model in the return type
    :doc-author: Trelent
    """
...
async def request_email(body: RequestEmail, background_tasks: BackgroundTasks, request: Request,
    ...
    with that email address and whether or not it has been confirmed yet. If it has been confirmed, we return
    a message saying so; otherwise, we add a task for sending out an email confirmation request.
